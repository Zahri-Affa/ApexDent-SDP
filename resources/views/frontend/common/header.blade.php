                ...
                <ul class="navbar-nav mx-lg-auto">
                    ...
                    <li class="nav-item dropdown">
                        ...
                        @foreach ($getLang as $key => $value)
                            @if($locale == $key)
                                <span  class="flag-icon {{ $flag[$key] }}"> </span> <span id="apexdent-flag-name-id">{{ $value }}</span> </a>
                            @endif
                        @endforeach
...
<li class="nav-item dropdown">
    @php
        ...
        $preferredOrder = ['en', 'ta', 'si'];
        
        // Sort the languages based on preferred order
        $sortedLangs = collect($getLang)->sortBy(function ($value, $key) use ($preferredOrder) {
            $index = array_search($key, $preferredOrder);
            return $index === false ? count($preferredOrder) : $index;
        });
    @endphp
    <a class="nav-link dropdown-toggle" href="#" id="dropdown09" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
        @foreach ($sortedLangs as $key => $value)
            @if($locale == $key)
                <span class="flag-icon {{ $flag[$key] }}"> </span> 
                <span id="apexdent-flag-name-id">{{ $value }}</span>
            @endif
        @endforeach
    </a>
    <div class="dropdown-menu" aria-labelledby="dropdown09">
        @foreach ($sortedLangs as $key => $value)
            <a class="dropdown-item" href="{{ route('lang.index', ['language' => $key]) }}">
                <span class="flag-icon {{ $flag[$key] }}"> </span>  {{ $value }}
            </a>
        @endforeach
    </div>
</li>
